...
stage ever rendered (632 of them), where the shipped composers draw 215. Both the licence exposure and
...
the shape. What composing *does* buy is concentration: 917 shipped assets reduce to **215 distinct
silhouettes**, because a family shape is shared by every weapon in it — and, since the vocabulary gate,
because an item that names no shape of its own draws what it *does* instead.
...
| **distinct across the project** | **917** | **215** |

`items/` reduces hard (62 slugs for 749 icons) because weapons share a family shape and everything else
shares a verb — the commission that buys them is
[commission-item-icons.md](commission-item-icons.md). `chars/` does not
reduce at all — `tests/char_compose_spec.lua` requires every blueprint to resolve to a *different*
silhouette, so that bucket is 153 drawings however it is sourced. It is also the most visible exposure
on screen, which is the argument for the board paintings having a date rather than an eventually.

### 215 is not the commission — 71 is

The buckets name 226 slugs between them, which are 215 distinct drawings, so 11 are **shared** and
...
| `traps/` | 2 |
| `props/` | 2 |
| shared by two or more | 11 |

**The 144 chars-only glyphs should not be commissioned at all.** Those tokens are interim by design and
...
the flat-glyph commission is **71 drawings**, with the board handled as paintings instead.

> **This was 268 drawings until the vocabulary gate landed** (2026-08-25). The item bucket alone was 262
> silhouettes, 164 of them drawn for exactly one item, because `tools/icons/map.lua` named a glyph per
> asset and the composer drew whatever it found. It is 62 now. The whole of that change is
> [the vocabulary](#the-vocabulary--the-set-of-shapes-this-game-may-draw).
...
`delapouite/claws` alone dresses 73 assets and `lorc/round-bottom-flask` 61. Draw **down** the list.
...
**Item icons (262 base silhouettes)** — **flat single-colour vector glyphs**, `viewBox="0 0 512 512"`
...
62 distinct silhouettes cover 749 item icons — 73 items share the claws, 61 the flask, 42 the ward.
That is inherent, and since the [vocabulary](#the-vocabulary--the-set-of-shapes-this-game-may-draw)
it is also deliberate: **the icon alone does not identify an item** in a full inventory, and it is
not asked to. Tooltips and names carry that weight. What the icon owes is the category — and the
tint, the aura and the pips split each of those shapes across a dozen readings, so a shelf of 62
drawings is nothing like a shelf of 62 pictures.
...
> swap of **215 base glyphs** — 62 of them for `items/` alone — not a commission of 749 icons; see
...
| | step | example |
|---|---|---|
| 1 | the weapon **family** — 15 shapes | an axe draws the battle-axe |
| 2 | its own **mapped glyph**, *if that glyph is in the vocabulary* | a censer draws the incense |
...
Step 2 is the gate: a glyph the vocabulary does not hold draws nothing and the item falls to step 3.
See [The vocabulary](#the-vocabulary--the-set-of-shapes-this-game-may-draw).
...
composer reads it there, which is what lets the ten wards be **one drawing in ten colours** instead of
ten drawings. On that path only, the three physical strike types carry a tint too (slash `#f2f2f2`,
...
### The vocabulary — the set of shapes this game may draw

`BASE_VOCABULARY` in `tools/icon_compose.lua` is the **declared list of silhouettes the composer is
allowed to name**, and step 2 above is the gate that enforces it. It is the difference between a
commission somebody chose and one that accumulated:

- **Before** — `tools/icons/map.lua` named a glyph per asset, 463 of them auto-guessed against 4,180
  game-icons names, and the composer drew whatever it found. 749 item icons resolved to **262
  silhouettes, 164 of them used exactly once**, and every new ability added another without anybody
  deciding to.
- **After** — 749 item icons resolve to **62**. An unapproved guess falls through to the verb, the
  hook or the type base, so the drawing set is a list you can read in one file and it does not grow
  with the catalogue.

The rule for admitting a mapped glyph is **four items**: below that the shape is naming an item rather
than a category, which is [the tooltip's job](#known-limitation-icon-reuse). The exceptions are the
handful of silhouettes worn by a **bound relic** — `BESPOKE`, capped at 24, because an uncapped
exception list is the tail again, arriving one reasonable-sounding decision at a time.

Three cases in `tests/art_pipeline_spec.lua` hold it: every slug an item draws is in the vocabulary,
no vocabulary entry is idle except a structural fallback, and the bespoke list stays under its cap.
The first stops the set growing, the second stops it carrying drawings nobody is waiting for.

A hand mapping in `tools/icons/overrides.lua` is still the way to correct a bad fall-through — but it
only bites if it names a slug **the vocabulary already holds**. `delapouite/caltrops` is a perfectly
good match for the caltrops and now draws nothing; the fix is to point that item at a shape the game
already buys (`lorc/mantrap`), not to add a drawing for one item.
...
unchanged. The 16 relics (`bound = true`) already keep their own silhouettes through the vocabulary's
`BESPOKE` list, and the cheaper route to a better relic icon is drawing the **base glyph** it composes
from, which improves every item riding on that glyph too.
