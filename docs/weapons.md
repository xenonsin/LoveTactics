...
| `rangeFromAdjacent = { type, tag }` | The cast **reaches as far as the neighbour does**: the longest *authored* `range` among the matching items beside it in the grid (`Combat.borrowedRange`, folded into `Combat.adjacencyRangeBonus` so every reader of reach gets it). Mark Target paints as far as the bow it needs. |
| `radiusFromAdjacent = { type, tag }` | The same borrow spent on a **ring** instead of a reach: the neighbour's range becomes the `aoe` radius (`Combat.borrowedRadius`, folded into `Combat.aoeRadius`). Clear Out spins as wide as the melee weapon beside it. |
