...
--
--   1. BASE    the item's OWN mapped glyph (tools/icons/map.lua), which no other item may draw.
--              Failing an entry: its weapon family, then what the ability DOES (VERB_BASE, off
--              Combat.abilityOutput) or when the charm FIRES (FIELD_BASE / HOOK_BASE), then the
--              ability cost-pool shape, then the generic TYPE_BASE.
--   2. TINT    the base is recoloured by the item's element/strike tag (ELEMENT_TINT) -- or, when it
--              names none, by the element of a status it applies, which is what makes a ward's colour
--              say which ward it is.
--   3. PIPS    `repRank` tier diamonds along the bottom, in the item's CLASS colour (the vendor shelf).
--
-- ONE ITEM, ONE SILHOUETTE. The map names a distinct glyph for every asset in the project -- 857 of
-- them, no two alike -- and `. icon-map` is what keeps it that way: a match there is a CLAIM, so the
-- second item reaching for the shield settles for its own second-best rather than sharing. The
-- channels below are therefore FALLBACKS and nothing else. They are what an item draws while nobody
-- has run the mapper since it was authored, and tests/art_pipeline_spec.lua reddens the moment two
-- items land on one of them.
--
-- (This replaced a VOCABULARY GATE, which held the catalogue to 62 silhouettes by refusing any mapped
-- glyph outside a declared list and letting the rest fall through to the structural channels. It bought
-- a small commission -- 62 drawings for 749 icons -- at the price of 73 items sharing the claws and 61
-- the flask, and that price is the one being paid back here. docs/art-assets.md records both sides.)
--
...
-- The per-item glyph map (tools/icons/map.lua, generated by `. icon-map` and hand-corrected through
-- tools/icons/overrides.lua). It is the FIRST channel and the only one that can say a thing about
-- this item rather than about its category, so it wins outright: a family shape is what a weapon
-- falls back to, not what it settles for. Optional -- absent map just means everything falls back
-- to the family/type bases below, and the uniqueness spec says so out loud.
...
-- Everything above is STRUCTURAL: a family, a type, a verb, a hook, a field. None of it identifies an
-- item -- a verb shape is shared by every ability that performs it -- so none of it is reached while
-- the map holds an entry for the asset. What it is for is the gap between authoring a blueprint and
-- running `. icon-map`: a new item draws something true about itself from the first frame, and the
-- uniqueness spec is what turns that placeholder into a red build rather than a shipped duplicate.
