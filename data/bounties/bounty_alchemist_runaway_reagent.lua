-- The Alchemist's opening posting: standing, always on the board, never spent.
--
-- A POSTING OF WORK THAT ALREADY EXISTED. `quest_alchemist_slot_01` owns the map, the fight and both
...
-- separate files.
--
-- WHY THIS IS THE PIECE. The vest is the only thing this work pays, and it is what the house wants back: the Crucible's
-- opener is a recovery, so the piece is the thing that was lost.
--
-- The work's other rewards are unchanged and still pay: the piece is a headline, not a narrowing
-- (models/bounty.lua's questFor).
--
-- STANDING, which is what an opener is for. It is the floor under the Alchemist's ladder -- a company
