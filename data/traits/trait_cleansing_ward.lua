...
-- "recipient" side of Trait.onStatusApplied), it strips exactly that status back off and then goes on
-- cooldown -- so the first debuff to touch a warded unit slides off, but a second within the cooldown
-- sticks. Removes only the status that just landed (ctx.status.id), not every debuff.
return {
    name = "Cleansing Ward",
    description = "Shrugs off the first debuff to touch you, then goes on cooldown.",
    cooldown = 20, -- ticks between triggers
    onStatusApplied = function(ctx)
        ...
        if ctx.onCooldown("trait_cleansing_ward") then return end
        ctx.setCooldown("trait_cleansing_ward", ctx.def.cooldown or 20)
