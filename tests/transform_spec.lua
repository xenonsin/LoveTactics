...
return {
    {
        ...
        fn = function()
            ...
            Transform.apply(c, knight, "pig")
            assert(knight.char.id == "pig", "the knight is wearing a pig")
    ...
    },
    {
        ...
        fn = function()
            ...
            assert(knight.char.id == "knight", "the knight is itself again")
    ...
    },
    {
        ...
        fn = function()
            local c = Combat.new(arena(8, 8), { unit("knight", 1, 1) }, { unit("bandit", 4, 4) })
            local knight = c.units[1]
            assert(Transform.apply(c, knight, "pig"), "the first shape takes")
            assert(Transform.apply(c, knight, "dire_bear") == nil, "the second is refused")
            assert(knight.char.id == "pig", "and it is still a pig")
        end,
    },
    {
        name = "Polymorph's status owns the shape: curing it turns the pig back",
        fn = function()
            local caster = armed("mage", { "ability_polymorph" })
            local c = Combat.new(arena(8, 8), { { char = caster, x = 1, y = 1 } }, { unit("bandit", 1, 3) })
            local mage, bandit = c.units[1], c.units[2]

            assert(Combat.useItem(c, mage, caster.inventory[1], 1, 3), "the cast resolves")
            assert(bandit.char.id == "pig", "the bandit is a pig")
            assert(Status.has(bandit, "polymorph"), "and carries the timer that owns the shape")

            -- Cure/Panacea route through Status.cleanse, which fires onExpire on the way out.
            Combat.cleanse(c, bandit)
            assert(not Status.has(bandit, "polymorph"), "the spell is broken")
            assert(bandit.char.id == "bandit", "and the pig is a bandit again")
        end,
    },
    {
        ...
        fn = function()
            ...
            Status.apply(c, bandit, "polymorph")
            assert(bandit.char.id == "pig", "the bandit is a pig")

            local result = Combat.dispel(c, { { x = 4, y = 4 } })
            assert(result.revealed == 1, "the dispel finds one illusion")
            assert(bandit.char.id == "bandit", "and tearing it down reverts the shape")
        end,
    },
    {
        ...
        fn = function()
            ...
            assert(boss.char.id ~= "pig", "a boss is never turned into livestock")
        end,
    },
    {
        name = "Wild Shape reserves mana for as long as the shape is worn, and reverting frees it",
        fn = function()
            local char = armed("archer", { "ability_wild_shape_bear" })
            local c = Combat.new(arena(8, 8), { { char = char, x = 1, y = 1 } }, { unit("bandit", 6, 6) })
            ...
            assert(hunter.char.id == "dire_bear", "the hunter is a bear")
            ...
            Status.remove(c, hunter, "wild_shape_bear") -- the timer running out
            assert(hunter.char.id == "archer", "the bear is an archer again")
    ...
    },
    {
        ...
        fn = function()
            local char = armed("archer", { "ability_wild_shape_wolf" })
            local c = Combat.new(arena(8, 8), { { char = char, x = 1, y = 1 } }, { unit("bandit", 6, 6) })
            local hunter = c.units[1]

            Combat.useItem(c, hunter, char.inventory[1], 1, 1)
            local weapon = Combat.defaultWeapon(hunter.char)
            assert(weapon and weapon.id == "fangs", "the wolf bites; it does not carry the hunter's bow")
        end,
    },
}
