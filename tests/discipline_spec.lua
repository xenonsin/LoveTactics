...
-- and "X's first subclass gate" are the same set. See data/quests/champions_challenge.lua.
...
tests[#tests + 1] = { name = "a newly unlocked discipline is pending at its parent vendor, once", fn = function()
    ...
    -- the fighter shelf.
    p.completedQuests.warlord_keep = true
...
tests[#tests + 1] = { name = "a multiclass announces at exactly one of its two parents", fn = function()
    ...
    for _, q in ipairs({ "warlord_keep", "held_position", "champions_challenge" }) do
