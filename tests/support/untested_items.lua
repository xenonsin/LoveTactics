-- The backlog of items that ship with no behavioural test, as of the day this ratchet was added.
--
-- This file exists so tests/item_coverage_spec.lua can be strict about NEW items without demanding
-- that 316 existing ones be written up first. Every id here is a debt, not a decision. The only
-- edit this file should ever get is a DELETION: write a test that names the item, then take its
-- line out. The spec fails if an id is listed here but has since gained a test, so the list cannot
-- quietly rot -- it ratchets down and never up.
--
-- See tests/item_coverage_spec.lua for what "has a test" means (and what it does not).

return {
    "ability_answering_din",
    "ability_arcane_cleave",
    "ability_backward_glance",
    "ability_binding_grace",
    "ability_blast_charge",
    "ability_blightstake",
    "ability_carved_stake",
    "ability_closed_ring",
    "ability_collapse",
    "ability_conjunction",
    "ability_consecrate",
    "ability_demon_cleave",
    "ability_desperate_strike",
    "ability_disarm",
    "ability_drowsing_air",
    "ability_en_garde",
    "ability_field_brew",
    "ability_gaff_line",
    "ability_gagging_storm",
    "ability_gaunt_vigil",
    "ability_ghost_kit",
    "ability_grasping_hollow",
    "ability_harrying_strike",
    "ability_held_reaction",
    "ability_indrawn_breath",
    "ability_invocation",
    "ability_keen_senses",
    "ability_kept_wound",
    "ability_leaping_crash",
    "ability_ledgers_due",
    "ability_magical_barrier",
    "ability_mana_sunder",
    "ability_march_wardens_standard",
    "ability_mark_of_heresy",
    "ability_muster_rift",
    "ability_not_yet",
    "ability_null_field",
    "ability_overcharge",
    "ability_power_strike",
    "ability_price_on_the_head",
    "ability_provoke",
    "ability_rally_banner",
    "ability_reckless_stance",
    "ability_reflect_magic",
    "ability_reflect_steel",
    "ability_renewal_banner",
    "ability_rimebite",
    "ability_riven_ground",
    "ability_sacred_banner",
    "ability_scouring_mercy",
    "ability_sealed_hour",
    "ability_set_charge",
    "ability_shared_burden",
    "ability_shieldbreak",
    "ability_silence",
    "ability_sleep",
    "ability_smite",
    "ability_snare_stake",
    "ability_splitglass",
    "ability_stand_down",
    "ability_stilled_hour",
    "ability_stillshade",
    "ability_straw_sentry",
    "ability_summon_golem",
    "ability_summon_homunculus",
    "ability_surge",
    "ability_thicketing",
    "ability_updraft",
    "ability_vanishing_strike",
    "ability_veil_of_night",
    "ability_warding_line",
    "ability_writ_of_fire",
    "ability_zealous_charge",
    "armor_adrenal_harness",
    "armor_aegis_unbidden",
    "armor_blindfold_cloak",
    "armor_blood_fever_mail",
    "armor_bogwalkers_coat",
    "armor_censer_cloth_habit",
    "armor_choking_apron",
    "armor_cuirass_of_the_roll",
    "armor_cutpurse_coat",
    "armor_everdraught_bandolier",
    "armor_gaunt_vigil_plate",
    "armor_gleaners_mantle",
    "armor_halting_rank",
    "armor_hem_of_the_stayed_hand",
    "armor_ichor_coat",
    "armor_interceding_stole",
    "armor_kennelbound_jerkin",
    "armor_kept_wound_shield",
    "armor_last_stand_plate",
    "armor_leaden_ward",
    "armor_martyrs_shield",
    "armor_mirrorsilk",
    "armor_muster_cuirass",
    "armor_opportunists_harness",
    "armor_overchannel_weave",
    "armor_quarryhide",
    "armor_rally_coat",
    "armor_raveners_hide",
    "armor_reagent_vest",
    "armor_reflecting_shield",
    "armor_reliquary_mantle",
    "armor_rimecloth",
    "armor_rimeguard",
    "armor_robes_unbidden",
    "armor_runed_plate",
    "armor_salamander_hide",
    "armor_sealed_coat",
    "armor_second_chance_vest",
    "armor_shared_bulwark",
    "armor_slipstep_leathers",
    "armor_smokecloth_wrap",
    "armor_stalkers_pelt",
    "armor_stormcloth",
    "armor_tower_shield",
    "armor_unlit_hood",
    "armor_unravelling_habit",
    "armor_unspent_heart",
    "armor_unyielding_harness",
    "armor_vestments_open_hand",
    "armor_volatile_carapace",
    "armor_wardens_oath",
    "armor_warding_chasuble",
    "armor_witchlight_shroud",
    "consumable_ball_bearings",
    "consumable_bannerets_steel",
    "consumable_berserkers_brew",
    "consumable_clearwater_vial",
    "consumable_crawler_mucus",
    "consumable_elixir_of_heroism",
    "consumable_elixir_of_the_adept",
    "consumable_elixir_of_the_giant",
    "consumable_flask_of_liquid_fire",
    "consumable_panacea",
    "consumable_scent_marker",
    "consumable_snare_stake",
    "consumable_spiteful_caltrops",
    "consumable_stamina_potion",
    "consumable_throwing_stone",
    "consumable_watchpost_draught",
    "consumable_wellspring_sandals",
    "consumable_wildcraft_poultice",
    "consumable_wine",
    "utility_aegis_of_the_oath",
    "utility_attunement",
    "utility_bloodstone_focus",
    "utility_brawlers_bandolier",
    "utility_burning_halo",
    "utility_censer_of_dawn",
    "utility_closed_entry",
    "utility_codex_of_hubris",
    "utility_codex_unanswered",
    "utility_crimson_standard",
    "utility_cutpurse_tally",
    "utility_deadhand_grip",
    "utility_demon_sigil",
    "utility_distant_sigil",
    "utility_drill_standard",
    "utility_duelists_poise",
    "utility_endurance",
    "utility_envious_glass",
    "utility_falconers_glove",
    "utility_feral_instinct",
    "utility_focus_stone",
    "utility_forty_one_marks",
    "utility_grace_reliquary",
    "utility_greyveil_cloak",
    "utility_greywatch_muster_roll",
    "utility_hallowed_censer",
    "utility_hour_returned",
    "utility_lent_aegis",
    "utility_mana_wellspring",
    "utility_marksmans_lens",
    "utility_martyrs_icon",
    "utility_maw_of_the_unfed",
    "utility_miasma_flask",
    "utility_miasmal_plate",
    "utility_names_he_kept",
    "utility_overwatch_scope",
    "utility_pale_vesture",
    "utility_red_thirst",
    "utility_relief_horn",
    "utility_relief_order",
    "utility_reliquary_of_tallies",
    "utility_reliquary_unbidden",
    "utility_reprisal",
    "utility_rooted_stance",
    "utility_sealed_reliquary",
    "utility_shield_bash",
    "utility_skirmishers_momentum",
    "utility_slipchain_charm",
    "utility_spellstrike",
    "utility_spirit_fetish",
    "utility_spiteful_ichor",
    "utility_stayed_hand",
    "utility_stormglass_rod",
    "utility_struck_name",
    "utility_tidewalker_boots",
    "utility_untroubled_mind",
    "utility_unyielding_seal",
    "utility_veterans_resolve",
    "utility_volatile_core",
    "utility_warding_censer",
    "weapon_answering_bell",
    "weapon_anvil_of_the_ninth",
    "weapon_apothecarys_lancet",
    "weapon_avalanche",
    "weapon_bell_hammer",
    "weapon_bellfounders_hammer",
    "weapon_bellowing_edge",
    "weapon_blight_spitter",
    "weapon_boar_spear",
    "weapon_carrion_axe",
    "weapon_censer_of_cold_light",
    "weapon_censer_of_the_grasping_hollow",
    "weapon_censer_of_the_hollow_dark",
    "weapon_censer_of_the_mustered_field",
    "weapon_censer_of_the_red_hour",
    "weapon_censer_of_the_unravelling",
    "weapon_conductor",
    "weapon_confessors_needle",
    "weapon_corvids_bow",
    "weapon_crozier",
    "weapon_cutpurse_knife",
    "weapon_deadfall_bow",
    "weapon_debt_bell",
    "weapon_disarming_pike",
    "weapon_drowned_censer",
    "weapon_duelists_edge",
    "weapon_emberwand",
    "weapon_envenomed_kris",
    "weapon_exposing_pike",
    "weapon_flame_fists",
    "weapon_forsworn_pike",
    "weapon_frost_fists",
    "weapon_frostfall_hammer",
    "weapon_gale_fists",
    "weapon_gathering_bell",
    "weapon_given_hour",
    "weapon_golem_fists",
    "weapon_gralloch_knife",
    "weapon_great_claws",
    "weapon_headsmans_cleaver",
    "weapon_hollow_arc",
    "weapon_iron_crook",
    "weapon_knell_point",
    "weapon_knell_shaft",
    "weapon_last_word",
    "weapon_ledgemans_axe",
    "weapon_lending_blade",
    "weapon_limning_bow",
    "weapon_litany_staff",
    "weapon_long_count",
    "weapon_long_fall",
    "weapon_long_silence",
    "weapon_mired_kris",
    "weapon_mired_maul",
    "weapon_nightjar",
    "weapon_pestilent_flail",
    "weapon_piercing_draw",
    "weapon_poachers_kris",
    "weapon_reapers_due",
    "weapon_reflecting_wand",
    "weapon_rending_claws",
    "weapon_renewal_staff",
    "weapon_rimebell",
    "weapon_rotting_claws",
    "weapon_sealed_censer",
    "weapon_sealed_hour",
    "weapon_sealed_ward_wand",
    "weapon_second_rank",
    "weapon_second_utterance_wand",
    "weapon_shepherds_crook",
    "weapon_sleepers_maul",
    "weapon_slow_verdict",
    "weapon_splitglass_saber",
    "weapon_splitting_maul",
    "weapon_stone_fists",
    "weapon_storm_fists",
    "weapon_struck_ledger",
    "weapon_sunderers_answer",
    "weapon_sunfall",
    "weapon_suspension_mace",
    "weapon_swineherds_wand",
    "weapon_sworn_lance",
    "weapon_tempo_debt",
    "weapon_the_stillness",
    "weapon_thin_place",
    "weapon_throughline",
    "weapon_tide_fists",
    "weapon_tidesbreak",
    "weapon_tinkers_maul",
    "weapon_tithe_axe",
    "weapon_unclosing_edge",
    "weapon_unravelling_shaft",
    "weapon_unravelling_wand",
    "weapon_unspent_blow",
    "weapon_vitriol_wand",
    "weapon_wardens_longbow",
    "weapon_wardens_tongue",
    "weapon_wetstone_mace",
    "weapon_whitening",
    "weapon_windward",
    "weapon_witchlight_bow",
    "weapon_wolfs_portion",
}
