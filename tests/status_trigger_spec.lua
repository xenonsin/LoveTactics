...
return {
    {
        ...
        fn = function()
            ...
            Status.apply(c, sick, "poison") -- any debuff arms the exploit
    ...
    },
    {
        ...
        fn = function()
            ...
            assert(not Status.has(burning, "burn"), "the burn is set off (consumed)")
        end,
    },
    {
        name = "Opportunist: afflicting a foe with a debuff hastes the bearer",
        fn = function()
            local bearer = withGrid("bandit", { "opportunists_charm" })
            local c = Combat.new(arena(8, 8), { unit(bearer, 1, 1) }, { unit("bandit", 1, 3) })
            local b = c.units[1]
            local foe = Combat.unitAt(c, 1, 3)
            assert(not Status.has(b, "hasted"), "the bearer starts un-hasted")
            Status.apply(c, foe, "mark", { applier = b })
            assert(Status.has(b, "hasted"), "inflicting a debuff seizes the opening (Haste)")
        end,
    },
    {
        name = "Cleansing Ward strips the first debuff to land, then must recharge",
        fn = function()
            local bearer = withGrid("knight", { "cleansing_ward" })
            local c = Combat.new(arena(8, 8), { unit(bearer, 1, 1) }, {})
            local b = c.units[1]
            Status.apply(c, b, "poison")
            assert(not Status.has(b, "poison"), "the first debuff is shrugged off")
            Status.apply(c, b, "cripple")
            assert(Status.has(b, "cripple"), "a second debuff within the cooldown sticks")
        end,
    },
    {
        name = "Executioner's Eye marks a foe the bearer stuns",
        fn = function()
            local bearer = withGrid("bandit", { "executioners_eye" })
            local c = Combat.new(arena(8, 8), { unit(bearer, 1, 1) }, { unit("bandit", 1, 3) })
            local b = c.units[1]
            local foe = Combat.unitAt(c, 1, 3)
            Status.apply(c, foe, "stun", { applier = b })
            assert(Status.has(foe, "mark"), "stunning a foe marks it for the kill")
        end,
    },
}
