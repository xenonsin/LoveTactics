...
return {
    ...
    {
        name = "the odds climb with the company, and stop short of both ends",
        fn = function()
            -- THE CURVE RUNS THE WRONG WAY ON PURPOSE: the fight you most want out of is the one you
            -- are least likely to escape. That is the wager the plate is quoting (models/flee.lua).
            ...
            -- not a decision, and a cornered party with no way out is a cutscene.
            ...
            -- A fight with no reading at all (an empty or unresolvable composition) still quotes the
            -- even number rather than refusing. There is nothing to compare, so nothing to adjust by.
    ...
    },
    {
        ...
        fn = function()
            ...
            -- inside the initiative countdown: the badge is on the token during the deploy phase, where
            -- a shuffle inside a hidden countdown would have shown nothing at all.
    ...
    {
        -- The status is one the game already teaches, which is the whole reason the plate needs no note
        -- of its own to name the stake: the hint line names it in words and the badge lands on the enemy
        -- token on the deploy screen, where the company can still read it before ringing the bell.
        name = "the catch is Hasted, cut to the opening",
        fn = function()
