-- Loadout pop-up panel: arrange a character's items in the 3x3 grid. This is where item positions
-- change (combat never reorders), reached from the hub (a building with panel = "loadout") and the
-- overworld (states/game.lua opens it directly). Hosts a ui/inventory_grid.lua editor plus a
...
-- ui/close_button.lua) and the three-input standard: mouse (click cells / arrows / X), keyboard
-- (arrows + Enter to move items, Q/E to switch character, Esc to close), gamepad (D-pad + A,
-- shoulder buttons to switch character, B to close).
--
...
function Loadout:draw()
    ...
    -- Legend for the connector-line colors.
    local ly = self.grid.y + self.grid.gridH + 16
    love.graphics.setFont(self.bodyFont)
    local lx = self.boxX + 30
    ...
    love.graphics.printf("Click / Enter: pick up then place    Q/E / shoulders: character    Esc / B: close",
...
function Loadout:mousepressed(x, y, button)
    ...
    if self.closeButton:mousepressed(x, y, button) then
        self:close()
    elseif #self.chars > 1 and pointIn(self.prevArrow, x, y) then
        self:switchChar(-1)
    elseif #self.chars > 1 and pointIn(self.nextArrow, x, y) then
        self:switchChar(1)
    elseif self.grid:mousepressed(x, y, button) then
        -- handled by the grid
    elseif not pointIn({ x = self.boxX, y = self.boxY, w = BOX_W, h = BOX_H }, x, y) then
...
function Loadout:keypressed(key)
    if key == "escape" then
        if not self.grid:cancelPickup() then self:close() end
    elseif key == "q" then
        self:switchChar(-1)
    elseif key == "e" then
        self:switchChar(1)
