...
local function ctxFor(combat, unit, trait, event)
    ...
    local ctx = {
        ...
        -- (There is no `fell` here. A scripted felling goes through Combat.fell, called from wherever
        -- the beat actually happens -- states/battle.lua's overrule sweep, or a status spending itself on
        -- its bearer's turn. It was briefly reachable from a trait hook, and a trait hook is the one
        -- place it must NOT be: every hook here fires inside the resolution of somebody else's action,
        -- so an act dispatched from one lands on top of the thing that triggered it. See
        -- data/traits/trait_boss_phases.lua's header.)
