...
-- HOW LONG THE SCHEDULE IS BEFORE IT COMES ROUND AGAIN.
--
-- The table was authored against a forty-day campaign with a deadline at the end of it, and that
-- deadline is retired -- a day is a real unit with no ceiling on it now (models/calendar.lua). So the
-- schedule is a SEASON that repeats rather than a countdown that runs out.
--
-- Cycling rather than re-keying off something else (bounties finished, say) because the whole value of
-- this table is that it is AUTHORED and learnable: the swamp shuts on the twenty-first and opens again
-- on the thirty-second, and a player who has been round once knows that. Re-keying it off a count the
-- player does not watch would keep the mechanism and throw away the reason for it.
BiomeWindow.SEASON = 40

-- The day the schedule is read at: the campaign's day, wrapped into the season. One-based, so day 40 is
-- the last row of the table rather than the first.
function BiomeWindow.seasonDay(day)
    day = math.max(1, math.floor(day or 1))
    return ((day - 1) % BiomeWindow.SEASON) + 1
end

-- The grounds reachable on `day`, sorted. THE tab row.
function BiomeWindow.openOn(day)
    local open = {}
    local d = BiomeWindow.seasonDay(day)
    for _, id in ipairs(BiomeWindow.ids()) do
        if BiomeWindow.isOpen(id, d) then open[#open + 1] = id end
    end
    return open
end
